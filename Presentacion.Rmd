...
ideal, con una formulación compleja y gran número de parámetros.
...
Esta propuesta, sin embargo, está basada en un modelo de medida más simple y con
más desarrollos teóricos previos. Esto nos permite formular un modelo más
parsimonioso, más sencillo de abordar teóricamente, y con un mayor soporte
teórico en lo que a la medición de rasgos de personalidad se refiere.
...
Asimismo, hay una cuasi-equivalencia con el modelo TIRT
...
```{r simulation_recovery_results_lt_prec, fig.height=4, fig.width=8, fig.align='center', dependson="simulation_recovery_result_matrix", cache=FALSE}
...
Fiabilidad decrece cuando sólo hay bloques homopolares
...
```{r simulation_recovery_results_correlations, fig.height=4, fig.width=8, fig.align='center', dependson="simulation_recovery_result_matrix", cache=FALSE}
...
Estructura del espacio latente se estima con mayor error cuando sólo hay bloques
homopolare, pero el algoritmo MCMC pierde menos precisión.
...
```{r simulation_recovery_results_lt_CrI, fig.height=4, fig.width=8, fig.align='center', dependson="simulation_recovery_result_matrix", cache=FALSE}
...
También, debido a la estimación conjnta, la incertidumbre en la estimación de
los parámetros de las personas, cuantificada según la cobertura de los
intervalos de credibilidad, es más precisa con el algoritmo MCMC.
Mientras que el análisis factorial confirmatorio infraestima la incertidumbre de
los parámetros, y además se ve afectado por algunas condiciones de estimación
como el número de bloques.
...
En un estudio empírico con el modelo Big Five, encontramos:

Mayores fiabilidades con el algoritmo MCMC

Aparentemente mejor convergencia:
En parámetros de escala cuando la información empírica es escasa
En los parámetros de correlación del espacio latente
